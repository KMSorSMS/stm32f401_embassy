...
static mut TASK3_STK: [OsStk; TASK2_STK_SIZE] = [0; TASK2_STK_SIZE];
...
fn main() -> ! {
    ...
    info!("task_3");
    unsafe{os_task_create(task_3, &mut TASK3_STK[TASK3_STK_SIZE-1], 23);}
    os_start()
}
...
fn task_3() {
    ...
    executor.run(|spawner| {
        ...
        unwrap!(spawner.spawn(dead_task3()));
    });
}
...
async fn blink5() {
    ...
    let end = Instant::now();
    let ms = end.duration_since(start).as_millis();
    info!("task_3_1 end with times:{}", count5_times);
    info!("task_3_1 execute time:{}ms", ms);
}
...
async fn blink6() {
    ...
    let end = Instant::now();
    let ms = end.duration_since(start).as_millis();
    info!("task_3_2 end with times:{}", count6_times);
